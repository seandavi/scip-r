...
| `tool_arguments` | list<string> | every `key=value` provenance stamp: `package`, `version`, `manager`, `parse_errors`, `parse_error_documents`, `git_commit`, `git_dirty`, `source_tarball`, `source_sha256`, Bioconductor/CRAN build fields (`git_url`, `git_branch`, `git_last_commit`, `date_publication`, `repository`, ...), and after `resolve`: `resolve_run_id`, `resolver`, `r_version`, `bioc_version` |
